...
Brushes.YellowGreen };
        ...
        public Bitmap flush(double step, double zhouqi, double pinlv)
        {
            bmap = new Bitmap(700, 200);
            gph = Graphics.FromImage(bmap);
             
            List<Point> MyPoint = new List<Point>();
            List<Point> CutLinePoint = new List<Point>();
            ...
            for (int j = 0; j < 10; j++)
            {
                ...
                { 
                    int sin = Convert.ToInt32(Math.Sin(i/zhouqi * 2 * Math.PI)*50);
                    p.X = (int)(j * zhouqi + i);
                    p.Y = 100 - sin;
                    MyPoint.Add(p);
                } 
            ...
            {
                gph.DrawLine(Mypen, MyPoint[i - 1], MyPoint[i]);
            }
            gph.DrawLine(black_pen, CutLinePoint[0], CutLinePoint[1]);
            ...
            //gph.drawDrawLine(Pens.Black, left_down.X, left_down.Y, right_down.X, right_down.Y);
