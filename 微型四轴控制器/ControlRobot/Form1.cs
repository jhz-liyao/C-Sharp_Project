...
namespace ControlRobot
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            ccmd.run(ccmd.stopCMD, comboBox3.Text);
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                label1.Text = "前进";
                ccmd.run(ccmd.upCMD, comboBox3.Text);
            }
            else if (e.KeyCode == Keys.Down || e.KeyCode == Keys.S)
            {
                label1.Text = "后退";
                ccmd.run(ccmd.downCMD, comboBox3.Text);
            }
            else if (e.KeyCode == Keys.Left || e.KeyCode == Keys.A)
            {
                label1.Text = "左转";
                ccmd.run(ccmd.leftCMD, comboBox3.Text);
            }
            else if (e.KeyCode == Keys.Right || e.KeyCode == Keys.D)
            {
                label1.Text = "右转";
                ccmd.run(ccmd.rightCMD, comboBox3.Text);
       ...
       /* private void button5_Click(object sender, EventArgs e)
        ...
        {
            sp.Write(new byte[] { 0xFD, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0xF8 }, 0, 10);
        }

        private void top_Click(object sender, EventArgs e)
        {
            Int16 Speed = Int16.Parse(BaseSpeed.Text);
            Speed += 50;
            if (Speed > 1000)
                Speed = 1000;
            sp.Write(new byte[] { 0xFD, 0x01, (byte)'T', (byte)(Speed >> 8), (byte)Speed, 0x00, 0x00, 0xF8 }, 0, 8);
            BaseSpeed.Text = Speed.ToString();
        }
        ...
        private void down1_Click(object sender, EventArgs e)
        {
            ...
            if (Speed < 0)
                Speed = 0;
            BaseSpeed.Text = (Speed).ToString();
            sp.Write(new byte[] { 0xFD, 0x01, (byte)'D', (byte)(Speed >> 8), (byte)Speed, 0x00, 0x00, 0xF8 }, 0, 8);
        }
